...
[Презентация проекта](https://docs.google.com/presentation/d/1wrWtKlJ6t-uNDlotf8tcV0omwv0LGLYXlW-9J3tu-mE/edit?usp=drive_link)  
...
### 2. Хранимая процедура sp_GetBookedRoomsReport. 
**Описание:** Процедура является источником данных для отчета **"Ведомость контроля бронирования и статуса прибытия гостей"**.  
Она предназначена для вывода следующей информации по отелю, категории и гостю:
•	Плановые даты заезда и выезда (из бронирования).  
•	Фактические даты заезда и выезда.  
•	Статус (Ожидается сегодня, 	Будущий резерв, Просрочен заезд).  

#### Входные параметры  
Процедура `sp_Hotel_Analytics` принимает три параметра. Параметр по категории номера является необязательным.

| Параметр | Тип данных | Значение по умолчанию | Описание |
| :--- | :--- | :--- | :--- |
| **@TargetDate** | `DATE` | `-` | Дата, на которую смотрим брони. |
| **@HotelID** | `NVARCHAR(50)` | `-` | Фильтр по отелям. |
| **@RoomTypeID** | `NVARCHAR(50)` | `NULL` | Фильтр по типам номеров. |
...
## 📈 Описание отчетов
В рамках проекта создано 4 отчета. Средством разработки отчетов является Power BI Desktop, MS Report Builder. 
...
![Отчет "Оплаты услуг по гостиницам в разрезе гостей"](screenshot/Оплаты%20услуг%20по%20гостиницам%20в%20разрезе%20гостей_1.png)

На второй странице отчета размещена круговая диаграмма, показывающая долю услуги в общей выручке.  

![Отчет "Оплаты услуг по гостиницам в разрезе гостей - доля услуги в общей выручке"](screenshot/Оплаты%20услуг%20по%20гостиницам%20в%20разрезе%20гостей_2.png)
...
![Отчет "Гео-аналитика"](screenshot/Гео-аналитика.png)
...
![Отчет "Комплексный Анализ"](screenshot/Комплексный%20Анализ%20доходности%20и%20эффективности.png)
...
![Отчет "Ведомость контроля бронирования"](screenshot/Ведомость%20контроля%20бронирования.png)
